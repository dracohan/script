...
#	}
	...
	if (!($returnVal4)) {	  
		...
		#DEBUG BEGIN
		#system ("cp /vobs/g2bsc/delivery/obj/$linkName $basePkg/$timestamp");
		#system ("cp $tempfile $basePkg/$timestamp");
		#system ("cp /vobs/g2bsc/delivery/obj/$targetName $basePkg/$timestamp");
		#system ("cp $logf $basePkg/$timestamp");
		#system ("cp $VCESPSFILE $basePkg/$timestamp");
		#DEBUG END
...
sub UpdateBlddef {
	...
	foreach(@_){
		...
		system("cp blddef.csv.new blddef.csv");
...
sub BuildRpm {
	...
	foreach my $i (0..$#files) {
		...
		$newname=$array[0]."\.$array[1]"."\.$array[2]";
		$patchnumber = $array[2];
